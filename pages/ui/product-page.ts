...
export class ProductPage extends ProductLocators {
  ...
  async fillQuantityInputDirectly(quantity: string): Promise<void> {
    await test.step('Fill quantity input directly and verify', async () => {
      ...
      // `fill` itself awaits actionability and the value being set; no extra hard sleep needed.
      // The follow-up `verifyFinalQuantityValueAfterFill()` performs a retrying
      // assertion via `assertHelper.assertNumberGreaterThanOrEqual`, which gives
      // the UI room to commit any normalization (min/max clamping).
      await expect.poll(() => this.inputQuantity.inputValue()).not.toBe('');
    });
  }
