 ...
 */
  ...
  /**
 * Click Delete button of the first address in the list.
 * Delegates to {@link clickDeleteAddressAt} for backwards-compatibility.
 */
  @step('Click Delete button of the address')
  async clickDeleteAddress(): Promise<void> {
    await this.clickDeleteAddressAt(0);
  }

  /**
   * Click the Delete button of the address at the given zero-based position
   * within the list. Use index `1` (or higher) to target a non-default address
   * once a second one has been added.
   * @param index Zero-based index of the Delete action button to click.
   */
