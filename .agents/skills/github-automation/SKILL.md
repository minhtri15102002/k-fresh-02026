...
When using `GITHUB_CREATE_AN_ISSUE_COMMENT` to post a PR Review, **act as a mentor, not just a strict gatekeeper:**
1. **Be Actionable & Exact:** Never say generic things like "Split this PR" or "Fix local lint errors." Instead, provide the **exact Bash/Git commands** to achieve it (e.g., `git reset origin/main`, `git checkout -b new-branch`, `npm run lint -- --fix`).
