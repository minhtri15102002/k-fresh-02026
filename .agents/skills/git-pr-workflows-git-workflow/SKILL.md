...
- Prompt: "Review all uncommitted changes for code quality issues. Check for: 1) Code style violations, 2) Security vulnerabilities, 3) Performance concerns, 4) Missing error handling, 5) Incomplete implementations. **Important Developer-Friendly Rule:** When generating feedback, act as a mentor. Provide highly actionable recommendations (e.g., exact bash/git commands to fix issues, exact auto-fix lint commands, and code snippets in `diff` format with explicit file paths and line numbers instead of generic warnings). Generate a detailed report with severity levels (critical/high/medium/low) and provide specific line-by-line feedback. Output format: JSON with {issues: [], summary: {critical: 0, high: 0, medium: 0, low: 0}, recommendations: []}"
...
> ⚠️ **Critical Pre-Branch Rule (learned from incident):** Before creating ANY new branch, you MUST sync the local target branch with the remote first. Skipping this causes the new branch to have zero divergence from `origin/main`, which can result in your feature commits being pushed directly onto `main` instead of the feature branch.
> ```bash
> git checkout main
> git pull origin main          # ← ALWAYS do this first
> git checkout -b feat/my-branch
> # ... make changes, commit ...
> git push -u origin feat/my-branch   # ← only pushes the feature branch
> ```
> Never branch from a detached HEAD or a stale local branch.
...
- **Always sync before branching**: Run `git checkout main && git pull origin main` before creating any feature branch. Never branch from a detached HEAD or a stale local ref — doing so creates a branch with no divergence from `origin/main`, causing commits to accidentally land on `main` on push.
- **Verify push destination**: After pushing, always confirm with `git log --oneline origin/main..origin/<your-branch>` that exactly your new commits appear ahead of main — nothing more, nothing less.
